...
### When User Provides URL/Website

**MANDATORY workflow for live website inputs**:

1. **Extract Design Tokens** using dembrandt CLI:
   ```bash
   npx dembrandt <url> --save-output --dtcg
   ```
   - Extracts: colors (palette + semantic), typography, spacing, borders, shadows, components
   - Output: `output/<domain>/TIMESTAMP.tokens.json` in W3C Design Tokens format
   - See `./references/dembrandt-usage.md` for CLI options

2. **Implement Code** following extracted tokens:
   - Use exact colors from token palette
   - Match typography specifications
   - Apply spacing and border tokens
...
1. **Extract Design Guidelines** using `./references/design-extraction-overview.md`:
   ...
   - Extract: colors (hex codes), typography, spacing scale, layout patterns
   - Document findings in project `docs/design-guidelines/extracted-design.md`
   - See `./references/extraction-prompts.md` for prompts
